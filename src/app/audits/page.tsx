...
export default async function AuditsPage({
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold text-card-foreground">Security Audits</h1>
          ...
          {SEVERITY_FILTERS.map((s) => {
            ...
            return (
              <Button
                ...
                size="sm"
                asChild
                className={active ? "bg-destructive/10 text-destructive border-destructive hover:bg-destructive/20" : ""}
              >
                <Link href={`/audits?${newParams.toString()}`} className="capitalize">
                  {s}
                </Link>
              </Button>
            );
      ...
      <p className="mb-4 text-sm text-muted-foreground">{result.total} audit reports</p>
        ...
        <div className="mt-8 flex justify-center gap-2">
          {page > 1 && (
            <Button variant="outline" size="sm" asChild>
              <Link href={`/audits?${baseParams.toString()}&page=${page - 1}`}>
                Previous
              </Link>
            </Button>
          )}
